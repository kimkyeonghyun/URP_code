...
# --- 질의 처리 및 평가 ---
def get_all_results(question, q_tokenizer, q_model, p_embs, passage_data, corpus, kiwi_tokenizer, retriever, device):
    """
    ...
    bm25_results = [passage_data[idx] for idx in retriever.get_top_n(tokenized_question, list(range(len(corpus))), len(corpus))[:50]]
...
def process_inference(inference):
    ...
    for item in split_list:
        if len(split_split) == 3:  # 최대 3개까지만 추가
            break
        if ':' in item:
            split_split.append(item.split(':')[0].strip())
    return split_split

def update_processed_inference(question_data, passage):
    """'processed_inference' 컬럼을 업데이트."""
    for idx in range(len(question_data)):
        if not question_data['processed_inference'][idx]:  # 비어 있는 경우 건너뜀
            continue

        # 'processed_inference'에서 법 이름과 조항을 추출하고 필터링
        split_laws_cleaned = [split_law_and_article(law) for law in question_data['processed_inference'][idx]]
        law_list = []
        for law in split_laws_cleaned:
            # 'passage' DataFrame에서 필터링
            filter_passage = passage[
                        (passage['law_name'] == law[0]) & 
                        (passage['law_clause'].apply(lambda clause: law[1] in clause))]
            
            if not filter_passage.empty:
                law_list.append(filter_passage['passage'].iloc[0])
            else:
                continue

        # 'processed_inference' 업데이트
        question_data.at[idx, 'processed_inference'] = law_list
...
def main():
    ...
    # 특정 조항('제1조(' 또는 '제2조(' 포함) 제거
    passage = passage[~passage['law_clause'].str.contains(r'제(1|2)조\(', na=False)]
    passage = passage.reset_index(drop=True)
    ...
    question_data = update_processed_inference(question_data, passage)
    ...
    for idx in tqdm(range(len(question_data)), desc='Retrieving questions'):
        ...
        dpr_results, bm25_results = get_all_results(
            question_data['inference'][idx], q_tokenizer, q_model, p_embs, passage['passage'].tolist(), corpus, kiwi_tokenizer, retriever, device)
        ...
        # 처리된 추론 결과 병합
        inference_results = question_data['processed_inference'][idx]
        dpr_results = inference_results + dpr_results
        bm25_results = inference_results + bm25_results
        reranked_results = inference_results + reranked_results
