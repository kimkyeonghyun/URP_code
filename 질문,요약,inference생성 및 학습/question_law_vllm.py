...
if __name__ == '__main__':    
    ...
    df = pd.read_csv('./materials/law_hang_re_noname.csv', encoding='utf-8-sig', lineterminator='\n')
    
    prompt_lst = []
    for i in range(len(df)):
        if args.isTest == 'True' and i == 5: break  # for test
    
        law_info = df.loc[i]['passage']
        
        system_prompt = '주어진 법률 정보의 내용을 함축할 수 있는 질문 형태를 만드세요.'
        user_prompt = '주어진 법률 정보의 핵심 정보를 포함하여, 그 내용을 함축할 수 있는 질문 형태를 만드세요.'
        user_prompt = ' 법 이름을 직접 언급하지 말고, 법률 정보의 내용도 그대로 베끼지 마세요.'
        user_prompt = ' 전반적인 내용은 유지하되 그 형태는 다르게 질문을 총 세 개씩 만드세요.'
    ...
    if args.isTest == 'True':  # for test
        df = df[:5] 
    df['question'] = contents



    savename = f'law_hang_question'
    if args.isTest == 'True':
        savename += '_test'
    df.to_csv(f'./materials/{savename}.csv', encoding='utf-8-sig', index=False)